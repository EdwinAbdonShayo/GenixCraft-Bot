...
from arm_control import move_servos, grip, reset_arm, step_move, move_and_pick_from_ratio
    ...
    try:
        while True:
            ...
            if qr_center:
                ...
                dx_ratio = (qr_center[0] - frame_center[0]) / (w / 2)
                dy_ratio = (qr_center[1] - frame_center[1]) / (h / 2)
                dx_ratio = max(-1.0, min(1.0, dx_ratio))
                dy_ratio = max(-1.0, min(1.0, dy_ratio))

                move_and_pick_from_ratio(dx_ratio, dy_ratio)
